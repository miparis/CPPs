...
/*   Updated: 2026/05/26 10:46:43 by miparis          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Bureaucrat:: Bureaucrat(std::string _nName, unsigned int _nGrade): _name(_nName)
{
...
/*									increment & decrement						 */
void Bureaucrat::incrementGrade(unsigned int _value)
{
...
void Bureaucrat::decrementGrade(unsigned int _value)
{
	int _new = this->_grade + _value;
	if (_new > gradeLowest)
		throw Bureaucrat::GradeTooLowException();
...
void Bureaucrat::executeForm(AForm const & _form)
{
    try
	{
        // El burócrata intenta ejecutar el formulario pasándose a sí mismo (*this)
        _form.execute(*this);
		std::cout << "<" << this->getName() << "> executed <" << _form.getName() << ">" << std::endl;
    }
	catch (std::exception &e)
	{
        // Si no estaba firmado o el grado era muy bajo, se atrapa aquí
        std::cout << this->_name << " couldn't execute " << _form.getName() 
